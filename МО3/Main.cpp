...
double epsF = 1e-6; // точность метода штрафных/барьерных функций
...
int Prepare(vector<double>& x0)
{
	...
	double koef = 1;
	int iter = 0;
	while (flag && (func_g(D[1][0], D[1][1]) > 0 || func_g(D[2][0], D[2][1]) > 0) && iter < MAXITER)
	{
		koef /= 2;
		for (int i = 0; i < N; i++)
		{
...
int Reflection() // отражение
{
	...
	double koef = alpha;
	int iter = 0;
	while (flag && func_g(xNew[0], xNew[1]) > 0 && iter < MAXITER)
	{
		koef /= 2;
		for (int i = 0; i < N; i++)
		{
...
int Contraction(double fTemp) // сжатие
{
	...
	double koef = beta;
	int iter = 0;
	while (flag && func_g(xTemp[0], xTemp[1]) > 0 && iter < MAXITER)
	{
		koef /= 2;
		for (int i = 0; i < N; i++)
		{
